            ...
            <div class="container-xxl flex-grow-1 container-p-y">
              ...
              <div class="position-absolute top-20 start-50 translate-middle">
                <img src="image/SLSU_LOGO.png " alt="Assessment Image" class="img-fluid" style="max-width: 100%; max-height: 100%; margin-left: 200px;margin-bottom: 45px;">
                      ...
                      <br>
                      <h2 class="mb-3">EVENT SCHEDULE</h2>
                      <div class="row">
                          <?php
                          
                          // Retrieve learning materials from the database, sorted by date in ascending order
                          $sql = "SELECT * FROM event_schedule WHERE year_section = '$section' AND status = 'active' ORDER BY STR_TO_DATE(date, '%Y-%m-%d') ASC";
                          ...
                          if (mysqli_num_rows($result) > 0) {
                              ...
                              while ($row = mysqli_fetch_assoc($result)) {
                                  ...
                                  // Check if the event_schedule is expired
                                  if (time() > $expiration_timestamp) {
                                      // Update status to 'expired' in the database
                                      $update_sql = "UPDATE event_schedule SET status = 'expired' WHERE id = " . $row['id'];
                                      mysqli_query($conn, $update_sql);
                                      
                                      // Skip this event_schedule as it is expired
                                      continue;
                                  }
                                  
                                  ?>
                                  <div class="col-md-12">
                                        <div class="material-box text-dark" style="background-color:#00bfff; height: 175px;"> <!-- Adjust the height value as needed -->
                                            <div class="inner-box" style="background-color:#00ff9d;">
                                                <h1 style="font-size: 40px;color: black;"><b><?php echo $row['date']; ?></b></h1>
                                            </div>
                                            <p style="color: black;"><b><?php echo strtoupper($row['title']); ?></b></p>
