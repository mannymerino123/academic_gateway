...
$expiredQueueSql = "UPDATE q_cashier SET status = 'expired' WHERE status = 'active' AND time <= DATE_SUB('$dateTimeNow', INTERVAL 720 MINUTE)";
mysqli_query($conn, $expiredQueueSql);

// Fetch the number of active queues
$sqlNumQueues = "SELECT COUNT(*) AS num_queues FROM q_cashier WHERE status = 'active' AND s_stat = 'waiting'";
...
if(isset($_POST['serve'])){
    ...
    $sqlUpdateNserved = "UPDATE q_cashier SET s_stat = 'waiting' WHERE s_stat = 'Nserved'";
    if(mysqli_query($conn, $sqlUpdateNserved)){
        // Serve the next person in line
        $sqlServeNext = "UPDATE q_cashier SET s_stat = 'served',status = 'expired' WHERE s_stat = 'waiting' AND priority_queue_number = '$currentQUEUE'";
        if(mysqli_query($conn, $sqlServeNext)){
            header("Location: q_admin_cashier.php");
            exit();
        } else {
...
if(isset($_POST['nextq'])){
    $sqlN_served = "UPDATE q_cashier SET s_stat = 'Nserved' WHERE priority_queue_number = '$currentQUEUE'";
            ...
            <div class="container-xxl flex-grow-1 container-p-y">
              ...
              <div class="row">
                ...
                <div class="col-md-6 col-xl-5">
                  ...
                  <form style="text-align: center;" action="q_admin_cashier.php" method="post">
