<?php
    ...
    $currentAttemptSql = "SELECT MIN(num_attempts) AS min_attempts FROM q_cashier WHERE student_ID = '$studentID' AND status = 'active'";
    ...
    if($minAttempts === null) {
        ...
        switch ($minAttempts) {
            case 5:
                ...
                break;
            case 4:
                $ATsql = "UPDATE q_cashier SET status = 'expired', s_stat = 'served' WHERE student_ID = '$studentID' AND num_attempts = 5";
                mysqli_query($conn, $ATsql);
                $Attempts_left -= 2;
                break;
            case 3:
                $ATsql1 = "UPDATE q_cashier SET status = 'expired', s_stat = 'served' WHERE student_ID = '$studentID' AND num_attempts = 4";
                mysqli_query($conn, $ATsql1);
                $Attempts_left -= 3;
                break;
            case 2:
                $ATsql2 = "UPDATE q_cashier SET status = 'expired', s_stat = 'served' WHERE student_ID = '$studentID' AND num_attempts = 3";
                mysqli_query($conn, $ATsql2);
                $Attempts_left -= 4;
                break;
            case 1:
                $ATsql4 = "UPDATE q_cashier SET status = 'expired', s_stat = 'served' WHERE student_ID = '$studentID' AND num_attempts = 2";
                mysqli_query($conn, $ATsql4);
                $Attempts_left -= 5;
                header("Location: q_cashier.php");
                exit();
                break;
            default:
    ...
    if (isset($_SESSION['last_queue_time'])) {
        ...
        if ($timeSinceLastQueue < $queueDelay) {
            ...
            echo "<script>window.location.href = 'q_cashier.php';</script>";
            exit();
        }      
    }
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['QUEUE'])) {
        ...
        $sqlQueue = "SELECT MAX(priority_queue_number) as max_queue FROM q_cashier";
        ...
        if ($nextQueueNumber == 3999) {
            $resetSql = "UPDATE q_cashier SET priority_queue_number = 3000";
        ...
        $sql = "INSERT INTO q_cashier (student_ID, priority_queue_number, time, status, s_stat, num_attempts) 
