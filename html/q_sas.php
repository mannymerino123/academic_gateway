<?php
  ...
  $expiredQueueSql = "UPDATE q_sas SET status = 'expired', num_attempts = '0' WHERE s_stat = 'served' AND status = 'expired'  AND time <= DATE_SUB('$dateTimeNow', INTERVAL 720 MINUTE)";
  mysqli_query($conn, $expiredQueueSql);

  // KANI ONCE NGA MO ABOT NA GANI UG 12 hours ang QUEUE ja wala gihapon na serve matic mo EXPIRED SIJA
  $dateTimeNow = date("Y-m-d H:i:s");
  $expiredQueueSql = "UPDATE q_sas SET status = 'expired' WHERE status = 'active' AND time <= DATE_SUB('$dateTimeNow', INTERVAL 720 MINUTE)";
  mysqli_query($conn, $expiredQueueSql);

  //KANI kuhaon nijag ang pina ka gamay nga queue para i display mao nay himoon nga current queue
  $sqlcurrent = "SELECT MIN(priority_queue_number) AS minimum FROM q_sas WHERE status = 'active' AND s_stat = 'waiting'";
  ...
  $sqlserved = "SELECT MAX(priority_queue_number) AS max_queues FROM q_sas WHERE student_ID = '$studentID' AND status = 'active' AND s_stat = 'waiting'";
  ...
  $Sqlnext = "SELECT MIN(priority_queue_number) AS MINIMUM FROM q_sas WHERE status = 'active' AND s_stat = 'waiting'";
        ...
        <div class="layout-page">
          ...
          <div class="content-wrapper">
            ...
            <div class="container-xxl flex-grow-1 container-p-y">
              ...
              <form style="text-align: center;" action="q_sas_function.php" method="post">
